...
export default async function OrdersAdminPage(props: { searchParams: Promise<{ [key: string]: string | undefined }> }) {
  ...
  if (status) {
    if (status === 'FAILED') {
      where.status = { in: ['FAILED', 'CANCELLED', 'REJECTED'] };
  ...
  if (sort === 'price_high') orderBy = { totalAmount: 'desc' };
  if (sort === 'price_low') orderBy = { totalAmount: 'asc' };
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between border-t border-gray-200 bg-white px-4 py-3 sm:px-6">
            <div className="flex flex-1 justify-between sm:hidden">
              <Link
                href={page > 1 ? `?${new URLSearchParams({...searchParams, page: (page - 1).toString()}).toString()}` : '#'}
              ...
              <Link
                href={page < totalPages ? `?${new URLSearchParams({...searchParams, page: (page + 1).toString()}).toString()}` : '#'}
