...
export default function NewProductPage() {
  ...
  const totalStock =
    sizeVariants.length > 0 ? sizeVariants.reduce((sum, sv) => sum + sv.stock, 0) : null;
    ...
    <form onSubmit={handleSubmit} className="pb-24 relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-6xl">
        ...
        <div className="lg:col-span-2 space-y-6">
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 gap-3 mb-4">
                {images.map((img, idx) => (
                  ...
                  >
                    ...
                    {idx === 0 && (
                      <span className="absolute top-1.5 left-1.5 bg-brand-ink text-white text-[10px] px-1.5 py-0.5 rounded-full uppercase tracking-wider font-bold">
                        Cover
                      </span>
                    )}
                    <button
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:bg-gray-50 transition-colors">
                {!process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME ? (
                  <div className="mb-4 text-sm text-red-600 bg-red-50 p-3 rounded-lg text-left">
                    <strong>Cloudinary Error:</strong> Add{" "}
                    <code>NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME</code> to your environment variables.
                  </div>
                ) : (
                  <CldUploadWidget
                  ...
                  >
                    {({ open }) => (
                      <button
                        type="button"
                        onClick={() => open()}
                        className="mb-4 inline-flex items-center gap-2 px-4 py-2 bg-brand-rose/10 text-brand-rose font-semibold rounded-lg hover:bg-brand-rose/20 transition-colors"
                      >
                        Upload Files
                      </button>
                    )}
          ...
          {/* 4. Size Variants & Stock ← MOVED TO MAIN COLUMN */}
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
            <div className="flex items-start justify-between mb-5">
              <div>
                <h2 className="text-lg font-semibold text-gray-900">Size Variants &amp; Stock</h2>
                <p className="text-xs text-gray-500 mt-0.5">
                  Enable to set individual stock per size. Total stock is auto-calculated.
                </p>
              </div>
              {totalStock !== null && (
                <span
            ...
            {/* Manual stock input — shown only when size variants are OFF */}
            {sizeVariants.length === 0 && (
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="flex flex-wrap gap-3 max-h-48 overflow-y-auto p-1 scrollbar-thin scrollbar-thumb-gray-300">
                  {(searchColor.trim() === ""
                    ? POPULAR_COLORS
                    : ALL_COLORS.filter(c =>
                        c.label.toLowerCase().includes(searchColor.toLowerCase())
                      )
                  ).map((c, i) => (
                    <button
                      key={i}
                      ...
                      className={`relative w-10 h-10 rounded-full border shadow-sm transition-transform hover:scale-110 ${
                        selectedColor?.label === c.label
                          ? "ring-2 ring-brand-rose ring-offset-2 border-brand-rose"
                          : "border-gray-200"
                      }`}
                    ...
                    >
                      {selectedColor?.label === c.label && (
                        <span className="absolute inset-0 flex items-center justify-center">
                          <svg
                            className={`w-5 h-5 ${
                              c.hex === "#FFFFFF" || c.hex === "rgba(255, 255, 255, 0)" || c.hex === "#FFFF00"
                                ? "text-black"
                                : "text-white"
                            }`}
                            fill="none"
                  ...
                  {searchColor.trim() !== "" &&
                    ALL_COLORS.filter(c =>
                      ...
                      <p className="text-sm text-gray-500 w-full text-center py-4">
                        No colors found matching &quot;{searchColor}&quot;
                      </p>
                    )}
                </div>
                {selectedColor && (
                  <p className="text-sm font-medium text-brand-ink mt-2">
                    Selected: {selectedColor.label} ({selectedColor.hex})
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Link different colored products together on the storefront with the same Group ID.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-6">
          ...
          {/* Summary card — shown once size variants are active */}
          {sizeVariants.length > 0 && (
              ...
              <div className="space-y-1.5">
                {sizeVariants.map(sv => (
                  <div key={sv.id} className="flex items-center justify-between text-sm">
                    ...
                    <span className={`font-bold tabular-nums ${sv.stock === 0 ? "text-red-500" : "text-green-800"}`}>
                      {sv.stock}
                    </span>
                  </div>
                ))}
