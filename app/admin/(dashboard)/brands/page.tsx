...
import { Plus, Edit, Trash2 } from "@/components/admin/AdminIcons";
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-serif font-bold text-brand-ink">Brands</h1>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? (
                <tr>
                  <td colSpan={3} className="px-6 py-8 text-center text-sm text-gray-500">Loading brands...</td>
                </tr>
              ) : brands.length === 0 ? (
                <tr>
                  <td colSpan={3} className="px-6 py-8 text-center text-sm text-gray-500">No brands found.</td>
                </tr>
              ) : (
                brands.map((brand) => (
                  <tr key={brand.id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-6 py-4">
                      <p className="font-medium text-brand-ink">{brand.name}</p>
                      {brand.description && <p className="text-xs text-gray-500 mt-1 truncate max-w-xs">{brand.description}</p>}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-500 font-mono bg-gray-50/50 rounded inline-block mt-3 ml-6 mb-3">{brand.slug}</td>
