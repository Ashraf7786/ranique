...
// Seeded wishlist — first 6 products as demo
const DEMO_WISHLIST_IDS = [
  MOCK_PRODUCTS[0]?.id,
  MOCK_PRODUCTS[2]?.id,
  MOCK_PRODUCTS[4]?.id,
  MOCK_PRODUCTS[6]?.id,
...
export default function WishlistPage() {
  const [wishlistIds, setWishlistIds] = useState<string[]>(DEMO_WISHLIST_IDS as string[]);
          ...
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            <div>
              ...
              <p className="font-sans text-sm text-brand-slate">
                {wishlistProducts.length} item{wishlistProducts.length !== 1 ? "s" : ""} saved — in sabko order karna hai?
              </p>
            </div>
          ...
          <div>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 mb-10">
              {wishlistProducts.map((product) => (
                ...
                >
                  {/* Image placeholder */}
                  <div className="relative aspect-product bg-gradient-to-br from-brand-blush to-brand-rose-light/50 flex items-center justify-center">
