...
function mapBackendProduct(dbProduct: any, siblings: any[] = []): Product {
  ...
  return {
    ...
    rating: dbProduct.reviews?.length ? Number((dbProduct.reviews.reduce((acc: number, r: any) => acc + r.rating, 0) / dbProduct.reviews.length).toFixed(1)) : dbProduct.rating,
    reviewCount: dbProduct.reviews ? dbProduct.reviews.length : dbProduct.reviewCount,
